...
class GraspingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        partial2 = np.array(read_point_cloud((self.root / (partial_path + 'x.pcd')).as_posix()).points)
        complete2 = np.array(read_point_cloud((self.root / (partial_path + 'y.pcd')).as_posix()).points)

        return partial.astype(np.float32), complete.astype(np.float32), partial2, complete2
